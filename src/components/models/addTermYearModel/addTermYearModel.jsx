...
        } else {
          setLoading(false);
          setExist("Term Year Update Failed");
        }
      });
    } else {
      // Pass only termyear for createTermYear
      const newTermYearData = { termyear: formData.termyear };
      createTermYear(newTermYearData, (res) => {
        if (res?.status === 201) {
          setLoading(false);
          changed();
          onHide();
...
          setLoading(false);
          setExist("Term Year Already Exists");
        } else {
          setLoading(false);
        ...
        <div className="d-flex flex-column">
          <div className="mt-3">
            <div className="has-validation">
              ...
              {validationMessage && (
                <div className="text-danger mt-1">{validationMessage}</div>
              )}
            </div>
          </div>
          ...
          <div className="text-danger text-center mt-4">{exist}</div>
        </div>
      </Modal.Body>
